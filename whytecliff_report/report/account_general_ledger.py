...
class whytec_general_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        ctx['account_code_id'] = data['form'].get('account_code_id', False) and data['form']['account_code_id']
        ctx['state'] = data['form']['target_move']
        self.context.update(ctx)
        if (data['model'] == 'ir.ui.menu'):
            new_ids = [data['form']['chart_account_id']]
            objects = self.pool.get('account.account').browse(self.cr, self.uid, new_ids)
        return super(whytec_general_ledger, self).set_context(objects, data, new_ids, report_type=report_type)

    def __init__(self, cr, uid, name, context=None):
        if context is None:
            context = {}
        super(whytec_general_ledger, self).__init__(cr, uid, name, context=context)
        self.query = ""
    ...
    def _get_account_code(self, data):
        if data.get('form', False) and data['form'].get('account_code_id', False):
            return self.pool.get('account.account').browse(self.cr, self.uid, data['form']['account_code_id']).code
        return ''
    ...
    def get_children_accounts(self, account):
        if self.context.get('account_code_id'):
            account = self.pool.get('account.account').browse(self.cr, self.uid, self.context.get('account_code_id'), context=self.context)
        result = []
        ...
        for child_account in self.pool.get('account.account').browse(self.cr, self.uid, ids_acc, context=self.context):
            if child_account.user_type.report_type not in ['income','expense']:
                continue
            sql = """
                SELECT count(id)
...
class whytec_report_generalledger(osv.AbstractModel):
    _name = 'report.whytecliff_report.report_generalledger'
    _inherit = 'report.abstract_report'
    _template = 'whytecliff_report.report_generalledger'
    _wrapped_report_class = whytec_general_ledger
